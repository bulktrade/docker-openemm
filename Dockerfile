FROM centos:7
RUN yum install -y deltarpm ld-linux.so.2 sqlite glibc.i686 libxml2.i686 zlib.i686 mysql MySQL-python libxml2 wget logrotate crontabs
...
RUN tar -xvzf apache-tomcat-8.0.24.tar.gz && ln -s apache-tomcat-8.0.24 tomcat && rm apache-tomcat-8.0.24.tar.gz
...
ADD run.sh /run.sh
ADD setup-cron.sh /setup-cron.sh
